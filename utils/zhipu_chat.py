...
class ZhipuChat:
    ...
    def __init__(self, api_key: str = None, default_model: str = "glm-4.5", default_temp: float = 0.3):
...
if __name__ == "__main__":
    ...
    print("\n--- JSON 模式 ---")
    result_json = chat.ask_json(system_prompt, user_prompt)
    print(result_json)
    # 示例：自然语言输出
    print("\n--- 文本模式 ---")
    summary_prompt = "Please summarize the following tool usage results:\n" + json.dumps(result_json, indent=2)
    result_text = chat.ask_text(summary_prompt, system_prompt="You are a summarization expert.")
    print(result_text)
